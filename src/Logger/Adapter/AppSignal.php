...
class AppSignal extends Adapter
{
    ...
    public function push(Log $log): int
    {
        ...
        if ($curlError !== CURLE_OK || $httpCode === 0) {
            throw new Push("AppSignal push failed with curl error ({$curlError}): {$response}", 500);
        }

        if ($httpCode >= 400) {
            throw new Push("AppSignal push failed with status code {$httpCode}: {$curlError} ({$response})", $httpCode);
        }
